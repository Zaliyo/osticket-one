...
$info = ($_POST && $errors) ? Format::htmlchars($_POST) : [];
...
<table class="table" width="800" cellpadding="1" cellspacing="0" border="0" id="ticketInfo">
    <tr>
        <td colspan="2" width="100%">
            <h1>
                <a href="tickets.php?id=<?php echo $ticket->getId(); ?>" title="<?php echo __('Reload'); ?>"><i
                            class="refresh icon-refresh"></i></a>
                <b>
                ...
                <div class="pull-right">
                    <a class="action-button btn btn-danger btn-sm" href="tickets.php?a=print&id=<?php
                    echo $ticket->getId(); ?>"><i class="icon-print"></i> <?php echo __('Print'); ?></a>

                    <?php if ($ticket->hasClientEditableFields()
                        // Only ticket owners can edit the ticket details (and other forms)
                        && $thisclient->getId() == $ticket->getUserId()) { ?>
                        <a class="action-button btn btn-secondary btn-sm" href="tickets.php?a=edit&id=<?php
    ...
    <tr>
        <td colspan="2">
            ...
            foreach (DynamicFormEntry::forTicket($ticket->getId()) as $i => $form) {
              ...
              $answers = $form->getAnswers()->exclude(Q::any([
                'field__flags__hasbit' => DynamicFormField::FLAG_EXT_STORED,
                'field__name__in' => ['subject', 'priority'],
                Q::not(['field__flags__hasbit' => DynamicFormField::FLAG_CLIENT_VIEW]),
              ]));
            ...
            foreach ($sections as $i => $answers) {
              ?>
                <table class="custom-data table" cellspacing="0" cellpadding="4" width="100%" border="0">
                    <tr>
                        <td colspan="2" class="headline flush-left">
                        <?php echo $forms[$i]; ?></th></tr>
...
$ticket->getThread()->render(['M', 'R', 'user_id' => $clientId], [
...
<?php if ($errors['err']) { ?>
    <div class="alert alert-primary" role="alert" id="msg_error"><?php echo $errors['err']; ?></div>
<?php }
elseif ($msg) { ?>
    <div class="alert alert-primary" role="alert" id="msg_notice"><?php echo $msg; ?></div>
<?php }
elseif ($warn) { ?>
    <div class="alert alert-primary" role="alert" id="msg_warning"><?php echo $warn; ?></div>
<?php }
...
?></textarea>
            ...
            if ($messageField->isAttachmentsEnabled()) {
              print $attachments->render(['client' => TRUE]);
            } ?>
        </div>
        <?php
        if ($ticket->isClosed() && $ticket->isReopenable()) { ?>
            <div class="border alert alert-warning">
        ...
        <p style="text-align:center">
            ...
            <input class="btn btn-secondary btn-sm" type="reset" value="<?php echo __('Reset'); ?>">
            <input class="btn btn-danger btn-sm" type="button" value="<?php echo __('Cancel'); ?>"
